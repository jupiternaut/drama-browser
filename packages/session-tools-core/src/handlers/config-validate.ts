 ...
 *
 * Validates Drama configuration files.
...
export async function handleConfigValidate(
  ...
  const dramaAgentRoot = process.env.CRAFT_CONFIG_DIR || join(homedir(), '.drama-agent');
  ...
  switch (target) {
    case 'config': {
      const result = validateJsonFileHasFields(
        join(dramaAgentRoot, 'config.json'),
        ['workspaces']
      );
      return successResponse(formatValidationResult(result));
    }
    ...
    case 'preferences': {
      const result = validateJsonFileHasFields(
        join(dramaAgentRoot, 'preferences.json'),
        []
      );
      return successResponse(formatValidationResult(result));
    }
    ...
    case 'tool-icons': {
      const result = validateJsonFileHasFields(
        join(dramaAgentRoot, 'tool-icons', 'tool-icons.json'),
        ['version', 'tools']
      );
      return successResponse(formatValidationResult(result));
    }
