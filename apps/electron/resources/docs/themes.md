...
3. **Preset themes**: `~/.drama-agent/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.drama-agent/theme.json` - Override specific colors (app-level)
...
```
~/.drama-agent/workspaces/{id}/config.json
```
...
Create `~/.drama-agent/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.drama-agent/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.drama-agent/themes/{name}.json`
...
1. Create `~/.drama-agent/theme.json` for overrides or `~/.drama-agent/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.drama-agent/theme.json` for overrides, `~/.drama-agent/themes/` for presets)
